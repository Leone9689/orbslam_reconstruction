...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, Map *pMap,OnlineFusionViewerManipulated *pFastFusion, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
   mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys),
    mpMap(pMap), mnLastRelocFrameId(0),mpFastFusion(pFastFusion)
{
...
Frame* Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    /*for(int y=0;y<imDepth.cols;y++){      
        for(int x=0;x<imDepth.rows;x++){   
             float pz = imDepth.at<float>(x,y);  
              std::cout<<pz<<",";
        }
    }*/
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // insert Key Frame into fastfusion                              
    mpFastFusion->insertKeyFrame( pKF-> GetPose(), this->mImage, this->mImDepth ); 
